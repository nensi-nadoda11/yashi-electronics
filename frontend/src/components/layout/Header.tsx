...
const primaryNavLinks = quickLinks.filter((link) =>
  ["Home", "Products"].includes(link.label),
);
...
export function Header() {
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimeoutRef.current = window.setTimeout(() => {
      ...
      const targetUrl = nextSearch
        ? `/products?search=${encodeURIComponent(nextSearch)}`
        : "/products";
          ...
          <Link to="/" className="shrink-0">
            <div className="flex items-center gap-3 rounded-full border border-slate-200 bg-white px-3 py-2 shadow-sm">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-white">
                <img
                  src={logoImage}
                  alt="Yashi Electronics logo"
                  className="h-[120%] w-[120%] max-w-none object-contain object-center"
                />
              </div>
              <div>
